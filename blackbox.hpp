...
public:
    ...
    virtual double zero_oracle_dense(double* X, double* Y, int N,int start_point, double* weights = NULL) const;
    virtual double zero_oracle_sparse(double* X, double* Y, int* Jc, int* Ir, int N, double* weights = NULL) const;
    virtual double zero_component_oracle_dense(double* X, double* Y, int N, int start_point, double* weights = NULL) const = 0;
