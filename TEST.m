...
algorithm = 'DP_GD';
loop = 30; 
step_size_gd = 1.7 / (20 * L*log(4/0.01));
fprintf('Algorithm: %s\n', algorithm);
time_1 = 0;
hist = 0;
for i=1:10
...
epsilon = 1.0;
sigma =1; % 
step_size_T3 = 1 / (24 * L*log(4*loop/0.01));
batchsize = 200; 
clip = 1/300*sqrt(162*loop^2*sigma^2/(log(4*loop/0.01)^2*batchsize));        %4*L*R0+2*genD;
var = 1.0*clip*batchsize*sqrt(loop*log(N))/(N*epsilon);%0.68
var = var^2;
...
epsilon = 1.0;
sigma =1;
step_size_T2 = 1/12 *1 / (2 * L*log(4/0.01));          
batchsize1 = 200; 
clip1 = 1/18*(1/64.8)*sqrt(162*loop^2*sigma^2/(log(4/0.01)^2*batchsize1));   %2*L*R0;
var2 = 1.3*clip1*batchsize1*sqrt(loop*log(N))/(N*epsilon);  %0.9
var2 = var2^2;
...
epsilon = 1.0;
sigma =1;
step_size_dpsgd = sqrt(loop)^(-5/3) * 0.01^(0.5)*28 /7.5;           
batchsize1 = 200; 
clip1_dpsgd = sigma*(1/sqrt(loop)^(2/3))* 0.01^(-0.5) * 6/5;   %2*L*R0;
var1 = 2.0*5.29*clip1_dpsgd*24*sqrt(loop*log(N))/(N*epsilon)*sqrt(1.95)*2/3;%1.4
var1 = var1^2;
...
algorithm = 'CSGD';
sigma =1;
step_size = 1 / (20 * L*log(4/0.01));           
batchsize2 = 200;
clip2 = 1/18*sqrt(54*loop*sigma^2/(log(4*loop/0.01)*batchsize2));  
...
title('adult, real data','FontSize',16)
